...
class KimiProvider(BaseLLMProvider):
    ...
    async def chat(self, messages: list[dict]) -> str:
        if not self.is_available():
            raise RuntimeError("Kimi API key not configured")
        base_url = (settings.KIMI_BASE_URL or "https://api.moonshot.cn/v1").rstrip("/")
        ...
        try:
            ...
            raise RuntimeError("Kimi response missing choices[0].message.content") from exc
    ...
    def _parse_analysis(self, content: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Kimi returned non-JSON content; wrapping as summary")
            data = {"summary": raw}
        ...
        return {
            "summary": str(data.get("summary") or "Kimi completed analysis but did not provide a summary."),
